...
const routes = [
  { path: "/web-site/production/", heading: "Producción", action: "Ver La Ola Art Gallery ↗" },
  { path: "/web-site/en/production/", heading: "Production", action: "View La Ola Art Gallery ↗" },
] as const;

const unsupported = ["HeroSlideResolverTest", "lockForUpdate", "callbacks", "RLP / PROD / 003", "RLP / PROD / 004"];
  ...
  for (const route of routes) {
    for (const theme of ["light", "dark"]) {
      for (const width of [320, 390, 768, 1024, 1440]) {
        ...
        await expect(index.getByRole("link", { name: route.action, exact: true })).toHaveAttribute("href", "https://www.laolaart.com/");
...
test("keeps Production dossiers in reading order and one mobile column", async ({ page }) => {
  ...
  const index = page.locator('[data-territory-index="production"]');
  expect(await index.locator('[data-production-record] h2').allTextContents()).toEqual(["Águilas FC", "La Ola Art Gallery"]);
