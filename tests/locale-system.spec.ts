...
test("serves the URL-driven Spanish and English Home counterparts", async ({ page }) => {
  ...
  await expect(page.getByText("Software Developer", { exact: true })).toBeVisible();
  await expect(page.getByText("Backend · Datos · Sistemas", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("link", { name: "Ver trabajo", exact: true })).toHaveAttribute("href", "/web-site/blog/");
  ...
  await expect(page.getByRole("heading", { name: "Rafael López", exact: true })).toBeVisible();
  await expect(page.getByText("Software Developer", { exact: true })).toBeVisible();
  await expect(page.getByText("Backend · Data · Systems", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("link", { name: "View work", exact: true })).toHaveAttribute("href", "/web-site/blog/");
  await expect(page.getByRole("link", { name: "Download CV", exact: true })).toHaveAttribute("download", "");
});

test("uses real bidirectional locale anchors without future English routes", async ({ page }) => {
  for (const [route, current, alternate] of [[esHome, "ES", "EN"], [enHome, "EN", "ES"]] as const) {
    ...
    await expect(page.locator('a[href^="/web-site/en/blog"], a[href^="/web-site/en/work"], a[href^="/web-site/en/experience"], a[href^="/web-site/en/education"], a[href^="/web-site/en/contact"]')).toHaveCount(0);
